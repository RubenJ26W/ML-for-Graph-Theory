...
def normalize_adjacency(A):
    """
    Computes A_hat = D^{-1/2} (A + I) D^{-1/2}
    where A is a sparse adjacency matrix.
    """
    ...
    # 2. Degree vector: D_tilde[i] = sum_j A_tilde[i,j]
    D_tilde = np.array(A_tilde.sum(axis=1)).flatten()

    # 3. Compute D^{-1/2}, careful with zeros
    D_inv_sqrt = 1.0 / np.sqrt(D_tilde)
    D_inv_sqrt[np.isinf(D_inv_sqrt)] = 0.0

    # 4. Sparse diagonal matrix
    D_inv_sqrt_mat = diags(D_inv_sqrt)

    # 5. Normalized adjacency: D^{-1/2} A_tilde D^{-1/2}
